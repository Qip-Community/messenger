...
// DOM-переменные
let searchInput, authScreen, appEl, loginForm, registerForm, settingsModal, mainMenu, friendRequestsContainer;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  mainMenu = document.getElementById('main-menu-dropdown');
  ...
  document.getElementById('main-menu-btn')?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (mainMenu) mainMenu.style.display = mainMenu.style.display === 'none' ? 'block' : 'none';
  });
...
function renderFriendRequests() {
  ...
  if (incomingRequests.length > 0) {
    ...
    incomingRequests.forEach(req => {
      ...
      friendRequestsContainer.appendChild(row);
    });
  }
}
